...
**Status:** ✅ DONE — implementata in `feature/US-102-zero-analyzer-warnings`, migrazione dei provider funzionali a notifier di classe e flutter analyze a zero avvisi
...
> - **I sei `deprecated_member_use`** (`authState`, `currentUser`, `currentUserId`, `firestoreService`, `isarDatabase`, `sessionSync`) sono stati convertiti in `Notifier` di classe Riverpod (`AuthState`, `CurrentUser`, `CurrentUserId`, `FirestoreService`, `IsarDatabase`, `SessionSync`), eliminando i typedef deprecati emessi da build_runner e azzerando completamente gli avvisi dell'analyzer.
...
**Acceptance Criteria**
- [x] I sei typedef deprecati sono via, con i provider riscritti come notifier di classe e il codice rigenerato
...
- [x] `flutter test` resta verde senza che nessun test esistente venga modificato
- [x] Il baseline dell'analyzer è azzerato (0 avvisi netti)
